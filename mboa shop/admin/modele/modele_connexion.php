...
function oublier()
{
	...
	if (!empty($_GET['m_t']) AND !empty($_GET['m_m'])) {
		...
		$req=$db->prepare('SELECT * FROM utilisateurs_admin WHERE mail = ? AND tel = ? ');
	...
	if (!empty($_POST['mpo'])){
		...
		<h1 class="logo"><a href="index.php?un=accueil">MBOA SHOP Online...</a></h1>
			...
			<p>vous avez effectuer une requete afin de pouvoir changer de mot de passe.<br>
				...
				<a href="admin/index.php?un=connexion&i=o&m_m=<?php echo $destinataire;?>&m_t=<?php echo $_POST['tel'];?>">Modifier</a>
			</p>
			</article>
		</body>
		</html>
